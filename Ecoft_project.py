...
# Radar avec les moyennes standarsiser pour chque cluster : 
    
    # Cération du dataframe pour le radar plotly
    from sklearn.preprocessing import StandardScaler
    scaler = StandardScaler()
    
    
    df_sc = scaler.fit_transform(df.iloc[0:,1:])
    df_radar = pd.DataFrame(df_sc, columns = df.iloc[0:,1:].columns)
    df_radar = df_radar.assign(cluster=y_kmeans)
    df_radar = df_radar.groupby("cluster").mean().reset_index().melt(id_vars = "cluster")
 


    import plotly.express as px
    df_radar = df_radar
    fig = px.line_polar(df_radar, r="value", theta="variable", color="cluster", line_close=True,
